...
func load(plugins []string, rcf RegisterClientFunc, logger log.Logger) (int, error) {
	var errors []error
	loadedPlugins := 0
...
func open(pluginName string, rcf RegisterClientFunc, logger log.Logger) (err error) {
	defer func() {
		if r := recover(); r != nil {
			var ok bool
			err, ok = r.(error)

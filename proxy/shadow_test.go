...
func TestShadowMiddleware_erroredBackend(t *testing.T) {
	timeout := 100 * time.Millisecond
	p := ShadowMiddleware(
		delayedProxy(t, timeout, &Response{Data: map[string]interface{}{"sonic": 42}, IsComplete: true}),
		func(_ context.Context, _ *Request) (*Response, error) {
			return nil, errors.New("ignore me")
		},
	)
	mustEnd := time.After(5 * timeout)
...
func TestShadowMiddleware_partialTimeout(t *testing.T) {
	...
	p := ShadowMiddleware(
		delayedProxy(t, 5*timeout, &Response{Data: map[string]interface{}{"sonic": 42}}),
		delayedProxy(t, timeout/2, &Response{Data: map[string]interface{}{"sonic": 42}, IsComplete: true}))

...
func TestNewMergeDataMiddleware_ok(t *testing.T) {
	...
	p := mw(
		dummyProxy(&Response{Data: map[string]interface{}{"sonic": 42}, IsComplete: true}),
		dummyProxy(&Response{Data: map[string]interface{}{"turbo": true}, IsComplete: true}))
...
func TestNewMergeDataMiddleware_sequential(t *testing.T) {
	...
	endpoint := config.EndpointConfig{
		Backend: []*config.Backend{
			...
			{URLPattern: "/aaa/{{.Resp0_int}}/{{.Resp0_string}}/{{.Resp0_bool}}/{{.Resp0_float}}/{{.Resp0_struct.foo}}?x={{.Resp1_turbo}}"},
	...
	p := mw(
		...
		func(ctx context.Context, r *Request) (*Response, error) {
			...
			return &Response{Data: map[string]interface{}{"turbo": "foo"}, IsComplete: true}, nil
		},
		func(ctx context.Context, r *Request) (*Response, error) {
			checkBody(t, r)
		...
		func(ctx context.Context, r *Request) (*Response, error) {
			...
			checkRequestParam(t, r, "Resp0_bool", "true")
			checkRequestParam(t, r, "Resp0_struct.foo", "bar")
			checkRequestParam(t, r, "Resp1_turbo", "foo")
...
func TestNewMergeDataMiddleware_sequential_unavailableParams(t *testing.T) {
	...
	endpoint := config.EndpointConfig{
		Backend: []*config.Backend{
			{URLPattern: "/"},
			{URLPattern: "/aaa/{{.Resp2_sonic}"},
			{URLPattern: "/aaa/{{.Resp0_turbo}}?x={{.Resp1_turbo}}"},
		},
		Timeout: time.Duration(timeout) * time.Millisecond,
		ExtraConfig: config.ExtraConfig{
			Namespace: map[string]interface{}{
	...
	p := mw(
		dummyProxy(&Response{Data: map[string]interface{}{"sonic": 42}, IsComplete: true}),
		func(ctx context.Context, r *Request) (*Response, error) {
			if v, ok := r.Params["Resp0_sonic"]; ok || v != "" {
...
func TestNewMergeDataMiddleware_sequential_erroredBackend(t *testing.T) {
	...
	endpoint := config.EndpointConfig{
		Backend: []*config.Backend{
			{URLPattern: "/"},
			{URLPattern: "/aaa/{{.Resp0_sonic}}"},
			{URLPattern: "/aaa/{{.Resp0_sonic}}?x={{.Resp1_turbo}}"},
		},
		Timeout: time.Duration(timeout) * time.Millisecond,
		ExtraConfig: config.ExtraConfig{
			Namespace: map[string]interface{}{
...
func TestNewMergeDataMiddleware_mergeIncompleteResults(t *testing.T) {
	...
	p := mw(
		dummyProxy(&Response{Data: map[string]interface{}{"sonic": 42}, IsComplete: true}),
		dummyProxy(&Response{Data: map[string]interface{}{"turbo": true}, IsComplete: false}))
	mustEnd := time.After(time.Duration(2*timeout) * time.Millisecond)
	out, err := p(context.Background(), &Request{})
	if err != nil {
		t.Errorf("The middleware propagated an unexpected error: %s\n", err.Error())
...
func TestNewMergeDataMiddleware_partialTimeout(t *testing.T) {
	...
	mw := NewMergeDataMiddleware(&endpoint)
	p := mw(
		delayedProxy(t, time.Duration(timeout/2)*time.Millisecond, &Response{Data: map[string]interface{}{"sonic": 42}, IsComplete: true}),

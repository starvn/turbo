...
func TestNewHTTPProxy_ok(t *testing.T) {
	...
	backendServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = fmt.Fprintf(w, "{\"sonic\":42, \"turbo\":true, \"foo\": \"bar\"}")
	}))
	...
	tmp, ok := result.Data["sonic"]
	if !ok {
		t.Errorf("The proxy returned an unexpected result: %v\n", result)
	}
	sonicValue, err := tmp.(json.Number).Int64()
	if err != nil || sonicValue != 42 {
		t.Errorf("The proxy returned an unexpected result: %v\n", sonicValue)
	}
...
func TestDefaultHTTPResponseParserConfig_nopEntityFormatter(t *testing.T) {
	expected := Response{Data: map[string]interface{}{"sonic": "turbo"}, IsComplete: true}
	result := DefaultHTTPResponseParserConfig.EntityFormatter.Format(expected)
	if !result.IsComplete {
		t.Error("unexpected result")
	}
	d, ok := result.Data["sonic"]
	if !ok {
		t.Error("unexpected result")
	}
	if v, ok := d.(string); !ok || v != "turbo" {
		t.Error("unexpected result")
	}
}
